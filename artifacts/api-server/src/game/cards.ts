...
export function buildInitialDecks() {
  const foodDeck: FoodCard[] = FOOD_TEMPLATES.map((tmpl, i) => ({ ...tmpl, id: `food-${i}`, type: "food" as const }));
  ...
  return { foodDeck: shuffle(foodDeck), materialEventDeck: shuffle(materialEventDeck) };
}
